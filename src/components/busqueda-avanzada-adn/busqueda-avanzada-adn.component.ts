...
import {
  ...
  OperatorContext,
  Option,
  BusquedaAvanzadaAdn30ClassNames,
  BusquedaAvanzadaAdn30Config,
  RemoveButtonContext,
  ArrowIconContext,
  Rule,
  RuleSet,
...
export const CONTROL_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => BusquedaAvanzadaAdn30Component),
  multi: true
};
...
@Component({
  selector: 'busqueda-avanzada-adn',
  templateUrl: './busqueda-avanzada-adn.component.html',
  styleUrls: ['./busqueda-avanzada-adn.component.scss'],
  providers: [CONTROL_VALUE_ACCESSOR, VALIDATOR]
})
export class BusquedaAvanzadaAdn30Component implements OnInit, OnChanges, ControlValueAccessor, Validator {
  public fields: Field[];
  public filterFields: Field[];
  public entities: Entity[];
  public defaultClassNames: BusquedaAvanzadaAdn30ClassNames = {
  ...
  @Input() emptyMessage: string = 'En un grupo de consulta no puede estar vacío. Por favor agregue una regla o Elimine todo el grupo...';
  @Input() classNames: BusquedaAvanzadaAdn30ClassNames;
  @Input() operatorMap: { [key: string]: string[] };
  @Input() parentValue: RuleSet;
  @Input() config: BusquedaAvanzadaAdn30Config = { fields: {} };
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (type === 'object') {
      ...
      throw new Error(`La 'configuración' esperada debe ser un objeto válido, obtenido ${type} en lugar.`);
    }
  }
  ...
  validate(control: AbstractControl): ValidationErrors | null {
    ...
    if (!this.config.allowEmptyRulesets && this.checkEmptyRuleInRuleset(this.data)) {
      errors.empty = 'Reglas vacías no están permitidas.';
      hasErrors = true;
    }
    ...
    if (type) {
      ...
      if (queryInput) {
        ...
        if (this.defaultTemplateTypes.indexOf(type) === -1) {
          console.warn(`No se pudo encontrar la plantilla para el campo con tipo: ${type}`);
        }
        return null;
      }
    }
